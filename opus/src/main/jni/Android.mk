...
include $(CLEAR_VARS)
LOCAL_PATH			:= $(ROOT)/libopusenc-0.2.1
LOCAL_MODULE        := opusenc

# $(addprefix ../, $(shell cd $(LOCAL_PATH)/../; find $(LOCAL_PATH)/src -type f -name '*.c'))
LOCAL_SRC_FILES := \
	$(shell cd $(LOCAL_PATH)/../; find $(LOCAL_PATH)/src -type f -name '*.c')

# LOCAL_LDLIBS        += -llog -lOpenSLES  -logg -lopus -L$(LOCAL_PATH)/lib
LOCAL_C_INCLUDES    := \
$(ROOT)/speexdsp-1.2.1/include \
$(ROOT)/speexdsp-1.2.1/libspeexdsp \
$(ROOT)/include \
$(LOCAL_PATH)/include \
